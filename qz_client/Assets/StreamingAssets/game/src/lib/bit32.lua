...
function bit32.extract( n, field, width )
	...
	return floor( n / 2^field ) % 2^width
end
...
function bit32.replace( n, v, field, width )
	...
	local w = 2^width
	local fw = f * w
	return ( n % f ) + ( v % w ) * f + floor( n / fw ) * fw
end
...
function bit32.lshift( x, disp )
	...
	return floor( x * 2^disp ) % 0x100000000
end
...
function bit32.arshift( x, disp )
	...
	elseif disp > 31 then
		...
		return floor( x / 2^disp ) + ( 0x100000000 - 2 ^ ( 32 - disp ) )
	end
end
...
function bit32.lrotate( x, disp )
	...
	local x = x * 2^disp
	return ( x % 0x100000000 ) + floor( x / 0x100000000 )
end
