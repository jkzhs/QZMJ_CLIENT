...
public class UIEventListener : MonoBehaviour
{
	...
	bool isColliderEnabled
	{
		get
		{
            PolygonCollider2D a = GetComponent<PolygonCollider2D>();
            if (a != null) return a.enabled;
	...
	void OnClick ()		
	{
		if (isColliderEnabled && onClick != null) {
			if (IsColor) {
				//CGameData.playSound (CGameData.PlayBtnSound);
			}
			onClick (gameObject);
		}
	}
	...
	void OnPress (bool isPressed)	{ 
		if (isColliderEnabled) {
			...
			pressScale (gameObject, isPressed);
		}
	}
	...
	{
		UIEventListener listener = GetListen (go);
		return listener;
	}

	static public UIEventListener Get (GameObject go, bool isColor)
	{
		UIEventListener listener = GetListen (go);
		listener.IsColor = isColor;
		return listener;
	}

	static private UIEventListener GetListen (GameObject go)
	{
	...
	public void pressScale (GameObject gameObj, bool isPressed) {
		if (IsColor == false) {
			return;
		}
		UISprite[] allSp = gameObj.GetComponentsInChildren<UISprite>(true);
		int len = allSp.Length;
		for (int i = 0; i < len; i++) {
			UISprite sp = allSp [i];
			Color spColor = sp.color;
			if (isPressed == false) {
				spColor = new Color (spColor.r / 0.7f, spColor.g / 0.7f, spColor.b / 0.7f, spColor.a);
			} else {
				spColor = new Color (spColor.r * 0.7f, spColor.g * 0.7f, spColor.b * 0.7f, spColor.a);
			}
			sp.color = spColor;
