...
function ProFi:checkMemory( interval, note )
	...
	self.memoryReports[#self.memoryReports+1] = memoryReport
...
function ProFi:writeReport( filename, mode )
	if #self.reports > 0 or #self.memoryReports > 0 then
		...
		self:writeReportsToFilename( filename,mode )
...
function ProFi:getFuncReport( funcInfo )
	...
	if not funcReport then
		...
		self.reports[#self.reports+1] = funcReport
	end
	return funcReport
end
...
function ProFi:writeReportsToFilename( filename, mode )
